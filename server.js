...
io.on('connect', socket => {
    ...
    socket.on("join room", (roomid, username) => {
        ...
        if (rooms[roomid] && rooms[roomid].length > 0) {
            ...
            socket.to(roomid).emit('message', `${username} 加入了会议`, 'Bot', moment().format(
                "HH:mm:ss"
            ));
    ...
    socket.on('disconnect', () => {
        ...
        socket.to(socketroom[socket.id]).emit('message', `${socketname[socket.id]} 退出了会议`, `Bot`, moment().format(
            "HH:mm:ss"
        ));
