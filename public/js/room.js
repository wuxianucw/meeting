...
function clearBoard() {
    if (window.confirm('确定要清空白板吗？（不可撤销！）')) {
...
function CopyClassText() {
    ...
    document.querySelector(".copycode-button").textContent = "已复制"
    setTimeout(()=>{
        document.querySelector(".copycode-button").textContent = "复制会议号";
    }, 5000);
}
...
continueButt.addEventListener('click', () => {
    ...
    document.querySelector("#myname").innerHTML = `${username} (自己)`;
    document.querySelector("#attendie-self").innerHTML = `${username} (自己)`;
...
document.querySelectorAll(".right-cont .head-title > div").forEach((element) => {
    element.addEventListener('click', () => {
        if (element.classList.contains('active')) return;
        document.querySelector('.right-cont .head-title .active').classList.remove('active');
        element.classList.add('active');
        document.querySelector('.right-cont .panel-cont.hidden').classList.remove('hidden');
        document.querySelector(`.right-cont .panel-cont.${element.dataset.oppo}`).classList.add('hidden');
    })
});
...
function handleGetUserMediaError(e) {
    switch (e.name) {
        case "NotFoundError":
            alert("未找到摄像头或麦克风，请检查设备是否连接");
            break;
        case "SecurityError":
        case "PermissionDeniedError":
            break;
        default:
            alert("打开摄像头或麦克风时出错：" + e.message);
            break;
    }
...
function handleVideoOffer(offer, sid, cname, micinf, vidinf) {
    ...
    connections[sid].ontrack = function (event) {
        ...
        const attendieId = `attendie${sid}`;

        if (!document.getElementById(attendieId)) {
            let attendie = document.createElement('div');
            attendie.classList.add('attendie');
            attendie.id = attendieId;
            attendie.innerHTML = `${cName[sid]}`;
            attendiesCont.appendChild(attendie);
        }
    ...
    connections[sid].onremovetrack = function (event) {
        ...
        if (document.getElementById(`attendie${sid}`)) {
            document.getElementById(`attendie${sid}`).remove();
        }
    };
